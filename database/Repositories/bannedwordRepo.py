...
# ---------------------------------------------------------------------------------------------------
    ...
    def get_banned_word(word):
        ...
        conn = db.get_connection()
        cur = conn.cursor()
        cur.execute("SELECT * FROM banned_words WHERE LOWER(word) = LOWER(%s) LIMIT 1;", (word,))
        row = cur.fetchone()
        cur.close()
        db.return_connection(conn)
        return row
    ...
    def exists_banned_word(word):
        ...
        conn = db.get_connection()
        cur = conn.cursor()
        cur.execute("SELECT 1 FROM banned_words WHERE LOWER(word) = LOWER(%s) LIMIT 1;", (word,))
